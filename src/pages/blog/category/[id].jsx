...
const PostsDetail = (props) => {
  ...
  return (
    <Layouts>
      ...
      <PageBanner pageTitle={postData.introTitle} breadTitle={postData.title} anchorLabel={"Publications"}
                  anchorLink={"#blog"} paddingBottom={1} />
              ...
              <div className="mil-adaptive-right mil-up">
                ...
                <ul className="mil-category-list">
                  {categories.map((item, key) => (
                    <li key={`categories-item-${key}`}><Link href={`/blog/category/${item.id}`}
                                                             className={item.id == props.id ? "mil-active" : ""}>{item.title}</Link>
                    </li>
                  ))}
